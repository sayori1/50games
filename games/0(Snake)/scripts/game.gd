...
class EatGenerator extends Node2D:
	var pos:Vector2
	var snake: Snake
	...
	func _draw() -> void:
		draw_rect(Rect2(pos.x * CELL_SIZE, pos.y * CELL_SIZE, CELL_SIZE, CELL_SIZE), Color.YELLOW, true)
...
class Snake extends Node2D:
	...
	var body: Array[Vector2] = [Vector2.ZERO, Vector2.ZERO + Vector2.RIGHT, Vector2.ZERO + Vector2.RIGHT * 2]
	...
	func _draw():
		for point in body:
			draw_rect(Rect2(point.x * CELL_SIZE, point.y * CELL_SIZE, CELL_SIZE, CELL_SIZE), Color.GREEN_YELLOW, true)
